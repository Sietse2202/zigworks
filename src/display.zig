...
pub const Color = packed struct(u16) {
    ...
    ///                                     ^ any hex color!
    /// ```
    pub fn fromRgb24Int(rgb: u24) Self {
...
pub const Rect = extern struct {
    ...
    /// }
    /// ```
    pub fn pushPixels(self: *const Self, pixels: []const Color) error{MismatchedSizes}!void {
        if (pixels.len != self.width * self.height)
            return error.MismatchedSized;

        self.pushPixelsUnchecked(pixels);
    }
    ...
    ///     // Draw 4 pixels in the top-left corner
    ///     const rect: Rect = .{ .x = 0, .y = 0, .width = 2, .height = 2 };
    ///     rect.pushPixelsUnchecked(&.{.black, .red, .green, .blue});
    ///
    ///     while (true) {};
    /// }
    /// ```
    ...
    /// Errors if the lengths aren't the same.
    pub fn pullPixels(self: *const Self, pixels: []Color) error{MismatchedSizes}!void {
        if (pixels.len != self.width * self.height)
            return error.MismatchedSized;

        self.pullPixelsUnchecked(pixels);
    }
...
pub const Point = struct {
    ...
    pub const top_left: Self = .{ .x = 0, .y = 0 };
    /// Top-right most corner of the screen
    pub const top_right: Self = .{ .x = screen_width, .y = 0 };
    /// Bottom-left most corner of the screen
    pub const bottom_left: Self = .{ .x = 0, .y = screen_height };
    /// Bottom-right most corner of the screen
    pub const bottom_right: Self = .{ .x = screen_width, .y = screen_height };
    ...
    pub fn drawString(
        ...
        if (options.centered) {
            ...
            const width_factor = if (options.large) large_font_width else small_font_width;
            const width = width_factor * text.len;
